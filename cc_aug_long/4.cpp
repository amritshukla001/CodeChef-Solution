...
void calc_mult() {
	for (int i = 1; i <= n; i++) for (int j = i; j <= n; j += i) fact[i].pb(j);
}


int find_posa(int c, int n) {
	vector<int> vpb = fact[c];
	int ans = 0;
	for (auto x : vpb) {
		if (x > n) break;
		if (c >= x) continue;
		int start = c;
		int temp = n / x;
		if (n % x != 0 && n % x >= (x - (x * temp - (c + (x * (temp - 1)))))) ans += (temp + 1);
		else ans += temp;
	}
	debug(vpb)
	debug(ans);
