...
void solve() {
	...
	sort(all(op), sortdesc);

	int start = op[0].second;
	int sum = op[0].first * (n - n / op[0].second);
	int rem = n / op[0].second;
	debug(rem)
	debug(sum)

	fo(i, 1, m) {
		int lcm = (start * op[i].second) / gcd(start, op[i].second);
		int rem1 = n / lcm;
		if (rem - rem1 > 0) sum += (op[i].first * (rem - rem1));
		rem = rem1;
		start = lcm;
		if (rem == 0) break;
	}
